-- ============================================================
-- Mapa de calor: usar la dirección como nombre del punto virtual
--
-- Para las geocercas sin nombre en GEMA (la mayoría), la etiqueta
-- pasa de "Punto 231 · KR 8-38B LA MAGDALENA" a mostrar la
-- dirección directamente: "KR 8-38B LA MAGDALENA" (la más
-- frecuente entre los eventos de la geocerca, descartando las
-- "SIN DEFINIR"). El código queda solo como último recurso, y las
-- 25 geocercas con nombre real en GEMA conservan su nombre.
-- Se redefinen las mismas funciones de la migración 052 cambiando
-- únicamente la expresión de la etiqueta.
-- ============================================================
  ...
  top_pv AS (
    SELECT d.cod_pv AS cod,
           COALESCE(
             ...
             -- 2) dirección GPS más frecuente dentro de la geocerca
             MODE() WITHIN GROUP (ORDER BY trim(d.direccion))
               FILTER (WHERE d.direccion IS NOT NULL AND trim(d.direccion) <> '' AND trim(d.direccion) NOT ILIKE 'SIN DEFINIR%'),
             -- 3) último recurso: el código
             'Punto ' || d.cod_pv
           ) AS nombre,
           SUM(d.dsub) AS suben,
           SUM(d.dbaj) AS bajan
...
-- Geocercas vistas en el rango, nombradas por su dirección cuando GEMA
-- no tiene el nombre configurado.
