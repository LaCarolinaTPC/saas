-- ============================================================
-- Mapa de calor: velocidad del bus al subir/bajar pasajeros
--
-- Cada evento de la registradora trae `velocidad`; se agrega a
-- las celdas como promedio de los eventos con movimiento de
-- pasajeros, para mostrarla en el globo del mapa (y detectar
-- recogidas con el bus en marcha). Se redefine get_mapa_calor
-- sobre la versión de la 056 añadiendo solo esa columna: el
-- arreglo compacto de celdas pasa a
-- [lat, lng, suben, bajan, punto_virtual|null, vel_prom|null].
-- ============================================================
...
CREATE FUNCTION get_mapa_calor(
  ...
  dets AS (
    SELECT ev.hora,
           ...
           ev.cod_pv, ev.punto_virtual, ev.direccion, ev.velocidad,
           ev.dsub, ev.dbaj
    FROM ev
