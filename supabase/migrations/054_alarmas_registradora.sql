-- ============================================================
-- Alarmas de la registradora (Rotación → Alarmas)
--
-- Las registradoras emiten, además de la telemetría, eventos de
-- alarma: bloqueos de puerta (P1/P2/P3), puerta abierta/cerrada
-- fuera de protocolo y fallas de comunicación. Ya llegan por el
-- sync crudo de `puntos_virtuales`; esta función los expone con
-- su ruta y conductor (LEFT JOIN: una alarma sin despacho activo
-- también cuenta) en un solo JSONB para la pantalla.
-- ============================================================
...
REVOKE ALL ON FUNCTION get_alarmas(DATE, DATE, TEXT, TEXT) FROM anon, public;
GRANT EXECUTE ON FUNCTION get_alarmas(DATE, DATE, TEXT, TEXT) TO authenticated, service_role;

-- Índice parcial: las alarmas son ~0,3% de la tabla; con esto la consulta
-- no escanea los millones de eventos de telemetría normal.
CREATE INDEX IF NOT EXISTS idx_pv_alarmas ON puntos_virtuales(fecha)
