...
| Homologaciones | `tipo_homologado` (código ORIGEN), `entidad_catalogo_id` (FK a `ausentismo_catalogos`), `entidad_nombre_recibido`, `pendiente_homologacion` | etapa 1 cuando es inequívoca |
...
| Liquidar | RRHH | persona resuelta, salario, fechas, entidad y tipo homologados, modalidad | liquidación explicada con versión; `valor_reclamado` **propuesto** (no aplicado) |
...
| **1. Motor de compatibilidad** | `motor.ts` con los 14 patrones; fixtures sintéticos con las 79 filas del anexo A y los 17 casos de 13.2; sin base de datos | ninguna | 1.027 aserciones verdes; Q70 y Q sin redondeo cubiertos; tolerancia técnica 1e-6 solo en importes |
