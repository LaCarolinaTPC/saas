...
**Fuente leída completa:** `docs/Especificacion_aplicacion_incapacidades_2024.md` (1.923 líneas, secciones 1–15 y anexos A–I con las 1.027 fórmulas de los 79 registros completos). **Fecha del plan:** 2026-09-10. **Fase 0 resuelta:** 2026-09-11 (sección 0). **Estado:** plan de desarrollo, sin código ni migraciones ejecutadas; listo para arrancar la fase 1. Nada de lo que aquí se llama tabla, columna, ruta o rol nuevo existe todavía en GESTIVO salvo que se marque **OBSERVADO en GESTIVO**.
...
| **12.18** (requisito nuevo, 2026-09-11) | El expediente debe permitir ajustar el salario y otros datos de la liquidación | «Estos expedientes deben permitir la regla de ajustar salario y otros.» | El expediente tiene **datos de liquidación ajustables** por RRHH, con motivo obligatorio, quién y cuándo, y cada ajuste genera una **liquidación nueva** (6.4) sin borrar la anterior. La matriz EPS no se modifica desde el expediente. Lista propuesta de campos ajustables (**por confirmar cuáles**): salario base y su vigencia; entidad pagadora homologada; tipo (EG/AT/EL/LM/LP) homologado; modalidad inicial/prórroga; días a cargo de la entidad (sobrescribir el cálculo, nunca por encima de los días totales); valor reclamado (sobrescribir el importe). Detalle en 6.1 y 7.3. |
...
| Ajustes de liquidación (12.18) | `modalidad_ajustada`, `dias_entidad_ajustados`, `valor_reclamado_ajustado`, y por cada ajuste `ajuste_motivo`, `ajustado_por_email`, `ajustado_at`; el histórico completo queda en `incapacidad_liquidaciones` (una fila por recálculo, con `entradas` y `es_vigente`) y en la bitácora | etapa 2, RRHH, con motivo obligatorio. Los valores recibidos de la matriz se conservan en `recibido_json`: el ajuste nunca los pisa |
...
| **2. Esquema, corte y etapa 1** | Migración: expediente, reglas + parámetro de corte, extensión del catálogo con `dias_min_cobro`, adjuntos, bucket, bitácora, vista; trigger de creación con corte + backfill de las **23** filas; módulo asignado a `rrhh` y `admin`; bandeja y expediente en solo lectura; alta manual; pantalla de Parámetros | 12.13 (borrador mínimo de estados si sigue abierta) | **Exactamente 23 expedientes** creados sin ningún campo de gestión; ninguna de las 626 anteriores tiene expediente; los cinco pagadores homologados sin pendientes; `anon`/`authenticated` reciben `permission denied`; una consulta deja rastro |
| **3. Etapa 2 RRHH: completar y liquidar** | Salario diligenciado por el funcionario (sugerencia de `employees` visible), homologaciones, liquidación explicada con la regla `gestivo-cobro-dias` | 12.7 (redondeo y moneda) | Liquidar sin salario devuelve error por campo; con datos completos reproduce el caso 13.2 elegido; recalcular deja historial |
| **4. Radicación y umbral** | Radicaciones con soporte; bandeja de cobro con importes y pestaña de no cobrables según el umbral **de cada entidad**; "cobrada" derivada; el informe de cobro pasa a leer `dias_min_cobro` | 12.10 | Duplicado de código rechazado; anulación conserva evidencia; el informe de cobro actual sigue funcionando con los mismos totales mientras las semillas sean 4/1 |
| **5. Recaudos, aplicaciones y conciliación** | Recaudos, aplicaciones, ajustes; saldo operativo; conciliación; todo operado por `rrhh` | 12.5, 12.6 | Abono parcial **no** cierra; Σ aplicaciones ≤ recaudo; sobrepago visible |
| **6. Consulta, tablero, exportaciones** | Submódulo `consulta` de solo lectura (para cuando se asigne a Revisoría), tablero desde el corte, CSV/Excel/PDF | — | `consulta` no puede invocar ninguna acción de escritura; tablero y listado coinciden para el mismo filtro |
| **7. Endurecimiento** | Retención de soportes, índices, paginación, documentación final y diccionario | — | 5.000 expedientes sintéticos: bandeja < 2 s; ninguna lista truncada a 1.000 |
